...
const businessUnitIds = resolveBusinessUnitSelection(args.bu ?? runSettings.defaultBusinessUnit ?? 'MAMMO', {
  urlKeySelection,
});
...
function baselineRowsForBusinessUnit(testCase, rows, businessUnit) {
  ...
  if (testCase.id === 'TC01') {
    const skippedTabs = new Set(
      (businessUnit.skippedTabs || []).map((tabName) => normalizeId(tabName)),
    );
    ...
    return rows.filter((row) => !skippedTabs.has(normalizeId(row['Tab Name'])));
  }
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>ALiS Individual Profile Baseline Playwright Test Report</title>
  <style>
    :root {
      --bg: #111;
      --panel: #171b22;
...
<body>
  <main>
    ...
    <section id="suite-panel" class="suite">
      <div class="suite-header">ALiS_IndividualProfile_Baseline</div>
...
function printAvailableOptions() {
  ...
  logger.info('');
  logger.info('Selection examples: --bu=MAMMO --url=NVRCP | --bu=all --url=all | --tc=TC01,TC04 | --flow=all | --project=chromium,firefox');
}
